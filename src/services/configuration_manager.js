...
export default class ConfigurationManager {
  ...
  fileWatcher() {
    ...
    if (prev.mtime !== curr.mtime) {
      const newConfigs = YAML.parseDocument(fs.readFileSync(GlobalConfigFilePath, 'utf8'))
      if (JSON.stringify(this.readAllConfigs().CORE) === JSON.stringify(newConfigs.toJSON().CORE)) {
