    ...
    if (Object.keys(this.cacheExpirary).length > 0) {
      ...
      for (const pair in this.cacheExpirary) {
        ...
        await Promise.allSettled([amm.getInputPrice(0, { d: Ethers.utils.parseUnits(this.pairAmountCache[pair], DEFAULT_DECIMALS) }),
          ...
          .then((values) => {
            if (!this.priceCache.hasOwnProperty(pair)) { this.priceCache[pair] = []; }
  ...
  async getPosition(wallet, pair) {
    try {
      ...
      clearingHouse.getPositionNotionalAndUnrealizedPnl(this.amm[pair],
        ...
        .then((values) => {
          positionValues.openNotional = Ethers.utils.formatUnits(values[0].value.openNotional.d, DEFAULT_DECIMALS);
  ...
  async getFundingRate(pair) {
    try {
      ...
      await Promise.allSettled([amm.getUnderlyingTwapPrice(3600),
        ...
        amm.nextFundingTime()])
        .then((values) => {
          funding.indexPrice = parseFloat(Ethers.utils.formatUnits(values[0].value.d));
