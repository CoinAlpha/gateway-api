...
export default class PerpetualFinance {
  providerUrl: string = XDAI_PROVIDER;
  provider: any = new Ethers.providers.JsonRpcProvider(this.providerUrl);
  gasLimit: number = GAS_LIMIT;
  contractAddressesUrl: string = CONTRACT_ADDRESSES;
  amm: Record<string, any> = {};
  priceCache: Record<string, any> = {};
  cacheExpirary: Record<string, any> = {};
  pairAmountCache: Record<string, any> = {};
  loadedMetadata: boolean = false;
  layer2AmbAddr: any;
  xUsdcAddr: any;

  constructor(private network = Networks.mainnet) {
    switch (this.network) {
      ...
      default:
        ...
        logger.error(err);
        throw new PerpetualFinanceError(err);
    }
  ...
  async load_metadata() {
    try {
      const metadata = await fetch(
        this.contractAddressesUrl
      ).then((res: Response) => res.json());
      const layer2: string[] = Object.keys(metadata.layers.layer2.contracts);
    ...
    if (Object.keys(this.cacheExpirary).length > 0) {
      ...
      for (let pair in this.cacheExpirary) {
        ...
        this.priceCache[pair][0] =
          this.pairAmountCache[pair] / Ethers.utils.formatUnits(values[0].d);
        this.priceCache[pair][1] =
          Ethers.utils.formatUnits(values[1].d) / this.pairAmountCache[pair];
      }
    }
  ...
  // get XDai balance
  async getXdaiBalance(wallet: Wallet) {
    try {
      const xDaiBalance = await wallet.getBalance();
      return Ethers.utils.formatEther(xDaiBalance);
    } catch (err) {
      logger.error(err);
      throw new PerpetualFinanceError(
        err.reason || 'error xDai balance lookup'
      );
    }
  }

  // get XDai USDC balance
  async getUSDCBalance(wallet: Wallet) {
    try {
      ...
      logger.error(err);
      throw new PerpetualFinanceError(err.reason || 'error balance lookup');
    }
  }

  // get  allowance
  async getAllowance(wallet: Wallet) {
