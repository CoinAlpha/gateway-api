...
import {
  encodePriceSqrt, getTickFromPrice,
...
export default class UniswapV3 {
  ...
  get_contract(contract, wallet) {
    ...
    if (contract === 'router') { return new ethers.Contract(this.router, routerArtifact.abi, wallet); }
  ...
  async currentPrice(wallet, tokenIn, tokenOut) {
    ...
    await Promise.allSettled(poolAddressRequests).then((values) => {
      for (pool = 0; pool < 3; pool++) {
        if (values[pool].value === ethers.constants.AddressZero) { poolPrices[pool] = 0; } else {
  ...
  async swapExactIn(wallet, baseTokenContractInfo, quoteTokenContractInfo, baseAmount, limitPrice, tier, _gasPrice) {
  ...
  async swapExactOut(wallet, baseTokenContractInfo, quoteTokenContractInfo, baseAmount, limitPrice, tier, _gasPrice) {
  ...
  async removePosition(wallet, tokenId) {
    ...
    return contract.multicall(data, { gasLimit: GAS_LIMIT });
  }
  ...
  async replacePosition(wallet, tokenId, token0, token1, amount0, amount1, fee, lowerPrice, upperPrice) {
    ...
    return contract.multicall(removeData.concat(mintData), { gasLimit: GAS_LIMIT });
  }
