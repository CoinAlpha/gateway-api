...
import {
  BigNumber, mulShift, Q32, ZERO, ONE, MaxUint256,
} from 'ethers';
...
export function getLiquidity(amount0, amount1) {
  return BigNumber.from(
    new bn(amount0.toString())
      .multipliedBy(amount1.toString())
      .sqrt()
      .toString(),
  );
...
export function mostSignificantBit(x) {
  ...
  for (const [power, min] of POWERS_OF_2) {
    if (JSBI.greaterThanOrEqual(y, min)) {
      y = JSBI.signedRightShift(y, JSBI.BigInt(power));
      msb += power;
    }
  }
  return msb;
...
export function getTickAtSqrtRatio(sqrtRatioX96) {
  ...
  return JSBI.lessThanOrEqual(getSqrtRatioAtTick(tickHigh), sqrtRatioX96)
    ? tickHigh
    : tickLow;
}
...
export function getTickFromPrice(price, tier, side) {
  ...
  if (tick <= getMinTick(tier)) { return getMinTick(tier); }
  return tick;
}
